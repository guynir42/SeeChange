...
def make_circle(radius, offset_x=0.0, offset_y=0.0, norm=1, imsize=None):
    """Create a small image of a circle centered around the middle of the image.

    Pixels that are not part of the circle will be set to zero.
    Pixels inside the shape will be set to 1.0.
    Pixels on the border will have a value between 0 and 1, depending on how much of the pixel is in.

    Parameters
    ----------
    radius: float
        The radius of the outer edge of the annulus.
    offset_x: float
        The offset in the x direction.
    offset_y: float
        The offset in the y direction.
    norm: int
        Normalization of the shape. Choose value:
        0- do not normalize, peak will have a value of 1.0
        1- normalize so the sum of the image is equal to 1.0
        2- normalize the squares: the sqrt of the sum of squares is equal to 1.0
    imsize: int or None
        Number of pixels on a side for the output.
        If None, will automatically choose the smallest odd integer that is larger than the outer radius.

    Returns
    -------
    output: array
        A 2D array of the circle or annulus.
    """
    if imsize is None:
        imsize = int(radius)
        if imsize % 2 == 0:
    ...
    # round radius to the given resolution:
    radius = np.round(radius / CACHED_RADIUS_RESOLUTION) * CACHED_RADIUS_RESOLUTION

    if radius in CACHED_CIRCLES:
        output = CACHED_CIRCLES[radius]
    else:
...
def make_annulus(inner, outer, offset_x=0.0, offset_y=0.0, norm=1, imsize=None):
    """Create a small image of a circle or annulus centered around the middle of the image.

    Pixels that are not part of the circle or annulus will be set to zero.
    Pixels inside the shape will be set to 1.0.
    Pixels on the border will have a value between 0 and 1, depending on how much of the pixel is in.
    ...
    ----------
    inner: float
        The radius of the inner edge of the annulus. Default is 0.0.
    outer: float
        The radius of the outer edge of the annulus.
    offset_x: float
        The offset in the x direction.
    ...
    circle1 = make_circle(radius=inner, offset_x=offset_x, offset_y=offset_y, norm=0, imsize=imsize)
    circle2 = make_circle(radius=outer, offset_x=offset_x, offset_y=offset_y, norm=0, imsize=imsize)

    output = circle2 - circle1
