...
def decam_ref_datastore( code_version, download_url, decam_cache_dir, data_dir, datastore_factory ):
    ...
    for ext in [ '.image.fits', '.weight.fits', '.flags.fits', '.image.yaml' ]:
        ...
        if not ext.endswith('.yaml'):
            ...
            if parse_env( "LIMIT_CACHE_USAGE" ):  # move it out of cache into the data directory
    ...
    with SmartSession() as session:
        ...
        if not parse_env( "LIMIT_CACHE_USAGE" ) and os.path.isfile( json_path ):
            ...
            if not parse_env( "LIMIT_CACHE_USAGE" ):  # save a copy of the image in the cache
